...
class BottomPlayWidgetState extends State<BottomPlayWidget>
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        final top = lerpDouble(miniBottom - _miniHeight, fullTop, t)!;
        final height = lerpDouble(_miniHeight, screen.height - fullTop, t)!;
          ...
          children: [
            ...
            Positioned(
              ...
              child: GestureDetector(
                ...
                onVerticalDragUpdate: (details) => _onDragUpdate(details, travel),
                    ...
                    children: [
                      ...
                      Positioned(
                        ...
                        child: IgnorePointer(
                          ...
                          child: Opacity(
                            opacity: fullOpacity,
                            child: _FullPlayerControls(con: con, song: song),
                          ),
                        ),
                      ),
                      Positioned(
                        ...
                        child: IgnorePointer(
                          ...
                          child: Opacity(
                            opacity: fullOpacity,
                            child: _FullPlayerInfo(con: con, song: song),
                          ),
                        ),
                      ),
                      Positioned(
  ...
  void _showMore(BuildContext context, dynamic song) {
    showModalBottomSheet<void>(
      ...
      builder: (_) => BottomSheetWidget(con: con, isNext: true, song: song),
    );
  }
    ...
    return Positioned(
      ...
      child: ClipRRect(
        ...
        child: CachedNetworkImage(
          imageUrl: imageUrl ?? '',
          fit: BoxFit.cover,
          memCacheWidth: (size * MediaQuery.of(context).devicePixelRatio).round().clamp(128, 1400),
          memCacheHeight: (size * MediaQuery.of(context).devicePixelRatio).round().clamp(128, 1400),
          placeholder: (_, __) => const LoadingImage(),
          errorWidget: (_, __, ___) => const ColoredBox(
            color: Color(0xFF151515),
            child: Center(child: Icon(Icons.music_note, color: Colors.white54, size: 48)),
          ),
        ),
      ),
    );
  ...
  Widget build(BuildContext context) => Column(
        ...
        children: [
          Text(
            ...
            maxLines: 2,
            overflow: TextOverflow.ellipsis,
            style: const TextStyle(color: Colors.white, fontSize: 28, height: 1.05, fontWeight: FontWeight.w800),
          ),
          ...
          Text(
            ...
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
            style: const TextStyle(color: Color(0xFFC9C9C9), fontSize: 18, fontWeight: FontWeight.w500),
          ),
          ...
          Row(
            children: [
              IconButton(
                ...
                icon: const Icon(Icons.thumb_up_alt_outlined, color: Colors.white, size: 29),
              ),
              IconButton(
                onPressed: () => _message(context, 'Комментарии пока недоступны'),
                icon: const Icon(Icons.chat_bubble_outline, color: Colors.white, size: 29),
              ),
              const Spacer(),
              IconButton(
          ...
          PositionSeekWidget(currentPosition: con.position, duration: con.duration, seekTo: con.seek),
        ],
      );

  void _message(BuildContext context, String message) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(AppLocale.text(message, message))));
  }
}
...
class PlayWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => SizedBox(height: 64, child: BottomPlayWidget(con: con));
}
