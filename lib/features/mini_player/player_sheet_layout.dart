...
class PlayerSheetLayout {
  ...
  double get artworkRadius => lerp(10, 20);

  double get sheetHeight => lerp(miniHeight, screenSize.height);
  double get sheetTop => screenSize.height - sheetHeight;

  double get miniTitleLeft => horizontalPadding + miniArtworkSize + 12;

  double get miniControlsRight => horizontalPadding;

  double get fullContentTop => artworkTop + artworkSize + 24;

  double get progressBottom => bottomSafeArea + 118;

  double get controlsBottom => bottomSafeArea + 28;

  double get scrimOpacity => interval(0.35, 0.85) * 0.35;
}
